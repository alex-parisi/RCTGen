/// Subposition.cpp
///
/// Subposition-data generator. Pure C-era math preserved verbatim - all
/// double-promotion semantics and macro semantics must remain identical
/// for byte-equivalent output. The file is wrapped in RCTGen namespace
/// and types renamed to the modern aliases (TrackPoint, kTileSize, etc).
...
*/
    ...
    const char *pitch_names[] = {
        "flat", "up12", "up25", "up42", "up60", "down12", "down25", "down42", "down60", "up75", "up90", "up105",
        "up120", "up135", "up150", "up165", "inverted", "down75", "down90", "down105", "down120", "down135", "down150",
        "down165", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",
        "", "up8", "up16", "up50", "down8", "down16", "down50"
    };
    const char *bank_names[] = {
        "unbanked", "left22", "left45", "right22", "right45", "left67", "left90", "left112", "left135", "left157",
        "right67", "right90", "right112", "right135", "right157"
    };
    ...
    double bank_angles[] = {
        0,M_PI_8, M_PI_4, -M_PI_8, -M_PI_4, 3 * M_PI_8, 4 * M_PI_8, 5 * M_PI_8, 6 * M_PI_8, 7 * M_PI_8, -3 * M_PI_8,
        -4 * M_PI_8, -5 * M_PI_8, -6 * M_PI_8, -7 * M_PI_8
    };
...
sprite_rotation_t base_sprite_rotations[708]=
    ...
    sprite_rotation_t inline_twist_sprite_rotations[40] = {
        ...
        {0, 0, 5,Y(0), 0, 3 * M_PI_8}, {8, 0, 5,Y(8), 0, 3 * M_PI_8}, {16, 0, 5,Y(16), 0, 3 * M_PI_8},
        {24, 0, 5,Y(24), 0, 3 * M_PI_8}, {0, 0, 10,Y(0), 0, -3 * M_PI_8}, {8, 0, 10,Y(8), 0, -3 * M_PI_8},
        {16, 0, 10,Y(16), 0, -3 * M_PI_8},
        {24, 0, 10,Y(24), 0, -3 * M_PI_8}, {0, 0, 6,Y(0), 0, 4 * M_PI_8}, {8, 0, 6,Y(8), 0, 4 * M_PI_8},
        {16, 0, 6,Y(16), 0, 4 * M_PI_8}, {24, 0, 6,Y(24), 0, 4 * M_PI_8}, {0, 0, 11,Y(0), 0, -4 * M_PI_8},
        {8, 0, 11,Y(8), 0, -4 * M_PI_8},
        {16, 0, 11,Y(16), 0, -4 * M_PI_8}, {24, 0, 11,Y(24), 0, -4 * M_PI_8}, {0, 0, 7,Y(0), 0, 5 * M_PI_8},
        {8, 0, 7,Y(8), 0, 5 * M_PI_8}, {16, 0, 7,Y(16), 0, 5 * M_PI_8}, {24, 0, 7,Y(24), 0, 5 * M_PI_8},
        {0, 0, 12,Y(0), 0, -5 * M_PI_8},
        {8, 0, 12,Y(8), 0, -5 * M_PI_8}, {16, 0, 12,Y(16), 0, -5 * M_PI_8}, {24, 0, 12,Y(24), 0, -5 * M_PI_8},
        {0, 0, 8,Y(0), 0, 6 * M_PI_8}, {8, 0, 8,Y(8), 0, 6 * M_PI_8}, {16, 0, 8,Y(16), 0, 6 * M_PI_8},
        {24, 0, 8,Y(24), 0, 6 * M_PI_8},
        {0, 0, 13,Y(0), 0, -6 * M_PI_8}, {8, 0, 13,Y(8), 0, -6 * M_PI_8}, {16, 0, 13,Y(16), 0, -6 * M_PI_8},
        {24, 0, 13,Y(24), 0, -6 * M_PI_8}, {0, 0, 9,Y(0), 0, 7 * M_PI_8}, {8, 0, 9,Y(8), 0, 7 * M_PI_8},
        {16, 0, 9,Y(16), 0, 7 * M_PI_8},
        {24, 0, 9,Y(24), 0, 7 * M_PI_8}, {0, 0, 14,Y(0), 0, -7 * M_PI_8}, {8, 0, 14,Y(8), 0, -7 * M_PI_8},
        {16, 0, 14,Y(16), 0, -7 * M_PI_8}, {24, 0, 14,Y(24), 0, -7 * M_PI_8}
    };
...
#define NUM_SPRITE_GROUPS 9
    ...
    const sprite_rotation_t *sprite_group_rotations[NUM_SPRITE_GROUPS] = {
        orthogonal_sprite_rotations, diagonal_sprite_rotations, turn_sprite_rotations, inline_twist_sprite_rotations,
        corkscrew_sprite_rotations, zero_g_orthogonal_sprite_rotations, zero_g_diagonal_sprite_rotations,
...
*/
    ...
    void calc_g_forces(TrackSection *track_section, double dist, int count) {
        double h = 0.001;
    ...
    coord_t get_subposition(Vector3 point, int view, int diag) {
        ...
        sub.z = (int) round(((16 * sqrt(6)) * (point.y) / kTileSize));
        int t;
        switch (view + 4 * diag) {
            case 0:
                sub.x = (32 - sub.x);
    ...
    int generate_view_subposition_data(TrackSection *track_section, int groups, int view, int reverse,
                                       subposition_t *points) {
        //Calculate start and finish angles
        TrackPoint start = track_section->curve(0);
        int start_angle = (int) roundf(4.0 * atan2(-start.tangent.x, start.tangent.z) / M_PI);
        if (start_angle < 0)start_angle += 8;
        TrackPoint end = track_section->curve(track_section->length);
        ...
        if (reverse) {
            ...
            reverse_offset.y = round(8 * reverse_offset.y / kClearanceHeight - (reverse - 1)) * kClearanceHeight / 8.0;
...
*/
        ...
        while (!done) {
            ...
            TrackPoint point = get_track_point(track_section, progress, reverse, reverse_transform, reverse_offset);
            ...
            if (((reverse || !skip_start) && (!reverse || !skip_final)) || progress != 0) {
                ...
                subposition_t point = {
                    cur_sub.x, cur_sub.y, cur_sub.z, (8 * view + rotation.yaw_sprite) % 32, rotation.pitch_sprite,
                    rotation.bank_sprite
                };
                points[count] = point;
                count++;
            ...
            do {
                ...
                TrackPoint point = get_track_point(track_section, progress + 0.01 * i, reverse, reverse_transform,
                                                   reverse_offset);
...
*/
    ...
    void calculate_energy_error(TrackSection *track_section, subposition_t *points, int num_points, const char *name) {
        //	for(int i=1;i<6;i++)
        //	{
        ...
        double correct_energy = 0.5 * (track_section->curve(track_section->length).position.y - track_section->curve(0).
                                       position.y) / kClearanceHeight;
        double actual_energy = fabs(energy / (16.0 * 6554.0 * 642000.0));
        double correct_length = track_section->length / kTileSize;
    ...
    void generate_subposition_data(TrackSection *track_section, const char *name, int groups, int reverse) {
    ...
    void get_angle() {
        float dist = (2.25) * kTileSize;
        float lower = 0 * kTileSize;
        float upper = 4 * kTileSize;

        for (int i = 0; i < 100; i++) {
            float mid = 0.5 * (lower + upper);
            TrackPoint point = large_zero_g_roll_left_curve(mid);
            if (point.position.z > dist)upper = mid;
            else lower = mid;
        }
        TrackPoint point = large_zero_g_roll_left_curve(0.5 * (lower + upper));

        float pivot = 0.116897727273 * kTileSize;

        float roll = atan2(point.normal.x, point.normal.y);
        float y_offset = cos(roll) * pivot * (1.0 / sqrt(
                                                  point.tangent.x * point.tangent.x + point.tangent.z * point.tangent.z)
                                              - 1.0);

        printf("Position xy: %f %f tiles Height %f clearances (offset %f)\n", point.position.x / kTileSize,
               point.position.z / kTileSize, point.position.y / kClearanceHeight,
               (point.position.y - y_offset) / kClearanceHeight);
        printf("Roll %f\n", roll * 180 / M_PI);
    }


    int run_subposition(int /*argc*/, const char ** /*argv*/) {
        //get_angle();
...
*/
	...
	generate_subposition_data(&(kTrackSections[S_BEND_LEFT_BANK]),"BankedSBendLeft",sprites,0);
	generate_subposition_data(&(kTrackSections[S_BEND_RIGHT_BANK]),"BankedSBendRight",sprites,0);

	sprites=SPRITE_GROUP_ORTHOGONAL|SPRITE_GROUP_ZERO_G_ROLLS_ORTHOGONAL;
	generate_subposition_data(&(kTrackSections[GENTLE_LEFT_BANK_TO_STEEP]),"LeftBankedUp25ToUp60",sprites,0);
	generate_subposition_data(&(kTrackSections[GENTLE_RIGHT_BANK_TO_STEEP]),"RightBankedUp25ToUp60",sprites,0);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_LEFT_BANK]),"Up60ToLeftBankedUp25",sprites,0);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_RIGHT_BANK]),"Up60ToRightBankedUp25",sprites,0);

	generate_subposition_data(&(kTrackSections[GENTLE_LEFT_BANK_TO_STEEP]),"Down60ToRightBankedDown25",sprites,33);
	generate_subposition_data(&(kTrackSections[GENTLE_RIGHT_BANK_TO_STEEP]),"Down60ToLeftBankedDown25",sprites,33);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_LEFT_BANK]),"RightBankedDown25ToDown60",sprites,33);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_RIGHT_BANK]),"LeftBankedDown25ToDown60",sprites,33);


	sprites=SPRITE_GROUP_DIAGONAL;


	generate_subposition_data(&(kTrackSections[GENTLE_LEFT_BANK_TO_STEEP_DIAG]),"DiagLeftBankedUp25ToUp60",sprites,0);
	generate_subposition_data(&(kTrackSections[GENTLE_RIGHT_BANK_TO_STEEP_DIAG]),"DiagRightBankedUp25ToUp60",sprites,0);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_LEFT_BANK_DIAG]),"DiagUp60ToLeftBankedUp25",sprites,0);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_RIGHT_BANK_DIAG]),"DiagUp60ToRightBankedUp25",sprites,0);

	generate_subposition_data(&(kTrackSections[GENTLE_LEFT_BANK_TO_STEEP_DIAG]),"DiagDown60ToRightBankedDown25",sprites,33);
	generate_subposition_data(&(kTrackSections[GENTLE_RIGHT_BANK_TO_STEEP_DIAG]),"DiagDown60ToLeftBankedUp25",sprites,33);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_LEFT_BANK_DIAG]),"DiagRightBankedDown25ToDown60",sprites,33);
	generate_subposition_data(&(kTrackSections[STEEP_TO_GENTLE_RIGHT_BANK_DIAG]),"DiagLeftBankedDown25ToDown60",sprites,33);
*/

        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::small_turn_left_steep)]),
                                  "LeftQuarterTurn3TilesUp60",SPRITE_GROUP_BASE, 0);
        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::small_turn_right_steep)]),
                                  "RightQuarterTurn3TilesUp60",SPRITE_GROUP_BASE, 0);
        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::small_turn_right_steep)]),
                                  "LeftQuarterTurn3TilesDown60",SPRITE_GROUP_BASE, 81);
        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::small_turn_left_steep)]),
                                  "RightQuarterTurn3TilesDown60",SPRITE_GROUP_BASE, 81);
        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::large_turn_left_to_diag_steep)]),
                                  "LeftEighthToDiagUp60",SPRITE_GROUP_BASE, 0);
        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::large_turn_right_to_diag_steep)]),
                                  "RightEighthToDiagUp60",SPRITE_GROUP_BASE, 0);
        generate_subposition_data(
            &(kTrackSections[std::to_underlying(TrackSectionId::large_turn_right_to_orthogonal_steep)]),
            "LeftEighthToDiagDown60",SPRITE_GROUP_BASE, 65);
        generate_subposition_data(
            &(kTrackSections[std::to_underlying(TrackSectionId::large_turn_left_to_orthogonal_steep)]),
            "RightEighthToDiagDown60",SPRITE_GROUP_BASE, 65);
        generate_subposition_data(
            &(kTrackSections[std::to_underlying(TrackSectionId::large_turn_left_to_orthogonal_steep)]),
            "LeftEighthToOrthogonalUp60",SPRITE_GROUP_BASE, 0);
        generate_subposition_data(
            &(kTrackSections[std::to_underlying(TrackSectionId::large_turn_right_to_orthogonal_steep)]),
            "RightEighthToOrthogonalUp60",SPRITE_GROUP_BASE, 0);
        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::large_turn_right_to_diag_steep)]),
                                  "LeftEighthToOrthogonalDown60",SPRITE_GROUP_BASE, 65);
        generate_subposition_data(&(kTrackSections[std::to_underlying(TrackSectionId::large_turn_left_to_diag_steep)]),
                                  "RightEighthToOrthogonalDown60",SPRITE_GROUP_BASE, 65);
...
int main(int argc, const char **argv) {
    return RCTGen::run_subposition(argc, argv);
}
