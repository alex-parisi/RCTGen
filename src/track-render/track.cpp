// Verbatim port of the C-era track implementation, kept as-is against the
// modernized track.h. All curve sampling, banking math, sprite emission,
// mesh transformation, and offset-table lookup is left exactly as written
// -- this file is the source of truth for byte-equivalent track-sprite
// generation against the goldens captured pre-modernization (see bisect:
// first bad commit was 5eff54f "Modernize iso-render", whose ripple
// touched this caller via the new inline vector operators). Any
// "modernization" of the math here (float-only literals like 0.5f * TILE_SIZE
// instead of 0.5 * TILE_SIZE, std::clamp(v, 0.0f, 1.0f), std::numbers::pi_v
// <float>, std::sqrt(float)) breaks byte-equivalence by 1 ULP on borderline
// curve sample points and propagates outward through the rasterizer.
