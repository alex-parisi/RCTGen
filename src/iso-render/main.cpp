...
int load_vector(vector3_t* vector, json_t* array)
{
    ...
    if (size != 3) { print_msg("Vector must have 3 components"); return 1; }
...
json_t* load_optional_array(json_t* json)
{
    ...
    {
        if (json_array_size(json) == 0) { print_msg("Empty array"); return nullptr; }
        json_incref(json);
        return json;
    }
...
{
    if (!json) { print_msg("Error: Property \"model\" not found"); return 1; }
    ...
    for (int i = 0; i < model->num_meshes; ++i)
    {
        ...
        if (mesh == nullptr) { print_msg("Error: Property \"mesh_index\" not found"); return 1; }
        ...
        for (int j = 0; j < static_cast<int>(json_array_size(mesh_arr)); ++j)
        {
            ...
            if (!json_is_integer(mesh_index)) { print_msg("Error: Property \"mesh_index\" is not an integer"); return 1; }
...
int load_lights(light_t* lights, int* lights_count, json_t* json)
{
    ...
    for (int i = 0; i < num_lights; ++i)
    {
        ...
        if (type == nullptr || !json_is_string(type)) { print_msg("Error: Property \"type\" not found or is not a string"); return 1; }
...
int load_project(project_t* project, json_t* json)
{
    ...
    if (items == nullptr || !json_is_array(items)) print_msg("Error: Property \"items\" does not exist or is not an array");
...
void project_add_model_to_context(project_t* project, context_t* context, model_t* model, int frame, int rotation)
{
    ...
    for (int i = 0; i < model->num_meshes; ++i)
    {
        ...
        const vector3_t orientation = model->orientation[i][frame] * static_cast<float>(std::numbers::pi / 180.0);
        const matrix_t rot = matrix_mult(rotate_y(orientation.x), matrix_mult(rotate_z(orientation.y), rotate_x(orientation.z)));
        const vector3_t pos = model->position[i][frame]
        ...
        for (std::uint32_t frame = 0; frame < project->items[i].frames; ++frame)
            for (std::uint32_t angle = 0; angle < project->items[i].rotations; ++angle)
            {
                ...
                if (project->items[i].frames > 1)
                    std::snprintf(relative_filename, 512, "%s%s_%u_%u.png", output_dir, project->items[i].name.c_str(), frame, angle + 1);
                else
                    std::snprintf(relative_filename, 512, "%s%s_%u.png", output_dir, project->items[i].name.c_str(), angle + 1);
...
int main(int argc, char* argv[])
{
    ...
    if (json_t* j = json_object_get(project_json, "base_directory"); j != nullptr && json_is_string(j)) base_dir = json_string_value(j);
    else std::printf("Error: No property \"base_directory\" found\n");

    const char* sprite_dir = nullptr;
    if (json_t* j = json_object_get(project_json, "sprite_directory"); j != nullptr && json_is_string(j)) sprite_dir = json_string_value(j);
    else std::printf("Error: No property \"sprite_directory\" found\n");

    const char* spritefile_in = nullptr;
    if (json_t* j = json_object_get(project_json, "spritefile_in"); j != nullptr && json_is_string(j)) spritefile_in = json_string_value(j);
    else std::printf("Error: No property \"spritefile_in\" found\n");

    const char* spritefile_out = nullptr;
    if (json_t* j = json_object_get(project_json, "spritefile_out"); j != nullptr && json_is_string(j)) spritefile_out = json_string_value(j);
    ...
    {
        if (!json_is_array(light_array)) { print_msg("Error: Property \"lights\" is not an array"); return 1; }
