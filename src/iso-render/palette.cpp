...
constexpr float vector3_get_luma(vector3_t c) noexcept
{
    return 0.299f * c.x + 0.587f * c.y + 0.114f * c.z;
}

inline float srgb2linear(float x) noexcept
{
    if (x <= 0.04045f) return x / 12.92f;
    return std::pow((x + 0.055f) / 1.055f, 2.4f);
}

inline float linear2srgb(float x) noexcept
{
...
{
    return vector3(srgb2linear(c.r / 255.0f), srgb2linear(c.g / 255.0f), srgb2linear(c.b / 255.0f));
}

color_t color_from_vector(vector3_t v)
{
    const auto to_u8 = [](float x) -> std::uint8_t {
        const float clamped = std::clamp(linear2srgb(x), 0.0f, 1.0f);
        return static_cast<std::uint8_t>(std::floor(clamped * 255.0f + 0.4999f));
    };
    return {to_u8(v.x), to_u8(v.y), to_u8(v.z)};
}

std::uint8_t palette_get_nearest(palette_t* palette, std::uint8_t region, vector3_t target, vector3_t* error)
{
    const region_t& reg = palette->regions[region];
    std::uint8_t nearest_index = reg.start_indices[0];
    float minimum_error = std::numeric_limits<float>::infinity();
    const int num_subregions = reg.subregions;
    for (int s = 0; s < num_subregions; ++s)
    {
        const std::uint8_t start_index = reg.start_indices[s];
        const std::uint8_t end_index = reg.end_indices[s];
        for (int i = start_index; i < end_index; ++i)
        {
            const color_t pal_col = reg.remap ? palette->remap_colors[i - start_index] : palette->colors[i];
            const vector3_t pal_vec = vector_from_color(pal_col);
            const float err = (target - pal_vec).norm();
    ...
    if (error != nullptr)
    {
        if (reg.remap)
        {
            *error = vector3(vector3_get_luma(target), 0.0f, 0.0f)
                   - vector3(vector3_get_luma(vector_from_color(palette->colors[nearest_index])), 0.0f, 0.0f);
        }
        else
        {
            *error = target - vector_from_color(palette->colors[nearest_index]);
        }
    }
...
palette_t palette_rct2()
{
    return palette_t{
        ...
        {{
            ...
            {67, 91, 91}, {83, 107, 107}, {99, 123, 123}, {8, 67, 8}, {16, 85, 16},
            {24, 103, 24}, {32, 121, 32}, {40, 139, 40}, {48, 157, 48}, {56, 175, 56},
            {64, 193, 64}, {72, 211, 72}, {80, 219, 80}, {88, 237, 88}, {92, 255, 92},
        }},
        {{
            {23, 35, 35}, {35, 51, 51}, {47, 67, 67}, {63, 83, 83},
            {75, 99, 99}, {91, 115, 115}, {111, 131, 131}, {131, 151, 151},
            {159, 175, 175}, {183, 195, 195}, {211, 219, 219}, {239, 243, 243},
