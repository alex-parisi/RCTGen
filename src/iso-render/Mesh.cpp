// Verbatim port of the C-era model implementation, kept as-is against the
// modernized model.h. All math (vertex/normal interpolation, smoothing
// thresholds, texture coordinate sampling) is left exactly as written --
// this file is the source of truth for byte-equivalent mesh loading
// against the goldens captured pre-modernization (see bisect: first bad
// commit was 5eff54f "Modernize iso-render"). Any "modernization" of the
// math here (float-only literals, std::sqrt(float) instead of unqualified
    ...
    int texture_load_png(Texture *texture, const char *filename) {
        ...
        for (int y = 0; y < texture->height; y++)
            for (int x = 0; x < texture->width; x++) {
                Color color = {
                    (uint8_t) row_pointers[y][4 * x], (uint8_t) row_pointers[y][4 * x + 1],
    ...
    int mesh_load_transform(Mesh *output, const char *filename, Matrix3 matrix) {
        ...
        for (size_t j = 0; j < scene->mNumMeshes; j++) {
            ...
            for (size_t i = 0; i < mesh->mNumVertices; i++) {
                output->vertices[mesh_start_vertex + i] = matrix_vector(
                    matrix, vector3(mesh->mVertices[i].x, mesh->mVertices[i].y, mesh->mVertices[i].z));
                output->normals[mesh_start_vertex + i] = matrix_vector(
                    matrix, vector3(mesh->mNormals[i].x, mesh->mNormals[i].y, mesh->mNormals[i].z));
                if (mesh->mTextureCoords[0]) {
                    output->uvs[mesh_start_vertex + i] = vector2(mesh->mTextureCoords[0][i].x,
